...
def test_change_observation():
    assert change_observation(
        np.ones((4, 4, 3)), obs_space_3d, (4, NUM_INDICATORS)
    ).shape == (4, 4, 3 + NUM_INDICATORS)
    assert change_observation(
    ...
    assert (
        change_observation(np.ones((3,)), obs_space_1d, (4, NUM_INDICATORS))[2] == 1.0
    )
    assert (
        change_observation(np.ones((3,)), obs_space_1d, (4, NUM_INDICATORS))[6] == 0.0
    )
    assert (
        change_observation(np.ones((3,)), obs_space_1d, (4, NUM_INDICATORS))[7] == 1.0
    )
    assert (
        change_observation(np.ones((3,)), obs_space_1d, (4, NUM_INDICATORS))[8] == 0.0
    )
    assert (
        change_observation(2, discrete_space, (4, NUM_INDICATORS))
        == 2 * NUM_INDICATORS + 4
    )
...
def test_get_indicator_map():
    ...
    assert (
        len(
            set(get_indicator_map(["bob_0", "joe_1", "fren_2", "joe_3"], True).values())
        )
        == 3
    )
