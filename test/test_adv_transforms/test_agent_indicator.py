from supersuit.adv_transforms.agent_indicator import change_obs_space,change_observation,get_indicator_map
from gym.spaces import Box, Discrete
import numpy as np
import pytest

obs_space_3d = Box(low=np.float32(0.),high=np.float32(1.),shape=(4,4,3))
obs_space_2d = Box(low=np.float32(0.),high=np.float32(1.),shape=(4,3))
obs_space_1d = Box(low=np.float32(0.),high=np.float32(1.),shape=(3,))
...
def test_obs_space():
    ...
    assert change_obs_space(obs_space_2d, NUM_INDICATORS).shape == (4,3,1+NUM_INDICATORS)
    assert change_obs_space(obs_space_3d, NUM_INDICATORS).shape ==  (4,4,3+NUM_INDICATORS)
