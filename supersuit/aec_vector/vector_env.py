...
class SyncAECVectorEnv(VectorAECEnv):
    ...
    def _collect_dicts(self):
        self.rewards = {
            agent: np.array(
                [env.rewards[agent] if agent in env.rewards else 0 for env in self.envs], dtype=np.float32)
            for agent in self.possible_agents
        }
        self._cumulative_rewards = {
            agent: np.array([env._cumulative_rewards[agent]
                            if agent in env._cumulative_rewards else 0 for env in self.envs], dtype=np.float32)
            for agent in self.possible_agents
        }
        self.dones = {
        ...
        self.infos = {agent: [env.infos[agent] if agent in env.infos else {
        } for env in self.envs] for agent in self.possible_agents}
    ...
    def observe(self, agent):
        ...
        for env in self.envs:
            obs = env.observe(agent) if agent in env.dones else np.zeros_like(
    ...
    def last(self, observe=True):
        passes = np.array(
            [env.agent_selection != self.agent_selection for env in self.envs], dtype=np.uint8)
