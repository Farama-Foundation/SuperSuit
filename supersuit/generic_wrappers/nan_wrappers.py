        ...
        def modify_action(self, action):
            if action is not None and np.isnan(action).any():
                ...
                if isinstance(obs, dict) and 'action mask' in obs:
                    warnings.warn(
                        "[WARNING]: Step received an NaN action {}. Environment is {}. Taking a random action from 'action mask'.".format(action, self))
                    action = self.np_random.choice(
                        np.flatnonzero(obs['action_mask']))
                else:
                    warnings.warn(
                        "[WARNING]: Step received an NaN action {}. Environment is {}. Taking a random action.".format(action, self))
...
def nan_noop_v0(env, no_op_action):
    def on_action(action, action_space):
        if np.isnan(action).any():
            warnings.warn(
                "[WARNING]: Step received an NaN action {}. Evironment is {}. Taking no operation action.".format(action, env))
            action = no_op_action
        return action
...
def nan_zeros_v0(env):
    def on_action(action, action_space):
        if np.isnan(action).any():
            warnings.warn(
                "[WARNING]: Step received an NaN action {}. Environment is {}. Taking the all zeroes action.".format(action, env))
