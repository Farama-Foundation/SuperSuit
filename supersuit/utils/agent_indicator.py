...
def change_obs_space(space, num_indicators):
    if isinstance(space, Box):
        ...
        if ndims == 1:
            pad_space = 255 * np.ones((num_indicators,), dtype=space.dtype)
            ...
            pad_space = 255 * np.ones(orig_low.shape[:2] + (num_indicators,), dtype=space.dtype)
