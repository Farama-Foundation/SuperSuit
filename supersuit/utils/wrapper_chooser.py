...
class WrapperChooser:
    def __init__(self, aec_wrapper=None, gym_wrapper=None, parallel_wrapper=None):
        assert (
            aec_wrapper is not None or parallel_wrapper is not None
        ), "either the aec wrapper or the parallel wrapper must be defined for all supersuit environments"
        ...
        if isinstance(env, gym.Env):
            ...
            if self.aec_wrapper is not None:
                ...
                return parallel_to_aec(
                    self.parallel_wrapper(aec_to_parallel(env), *args, **kwargs)
                )
