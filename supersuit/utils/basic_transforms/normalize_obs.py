...
def check_param(obs_space, min_max_pair):
    ...
    assert np.all(np.isfinite(obs_space.low)) and np.all(
        np.isfinite(obs_space.high)
    ), "Box observation_space of environment has infinite bounds! Only environments with finite bounds can be passed to normalize_obs_v0"
